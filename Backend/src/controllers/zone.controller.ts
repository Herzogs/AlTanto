...
const createZone = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
    ...
    const validData = req.body as IZoneRequest;
    console.log(validData.email, " Aca 21 controller");
    try {
        const zone = await zoneService.createZone(validData);
