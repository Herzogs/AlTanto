  ...
  try {
    ...
    if (response.status !== 201) {
      if (response.status === 401) {
        throw new Error(
          "Unauthorized: You are not authorized to create groups."
        );
      } else {
...
export const addUserToGroupWithCode = async ({
  groupId,
  userId,
  groupCode,
}) => {
  try {
    const response = await axiosInstance.post(`/group/${groupId}/add-user`, {
      groupId,
      userId,
      groupCode,
    });
...
export const sendSOS = async (groupId, userId, address) => {
  try {
    const response = await axiosInstance.post(`/group/sos`, {
      groupId,
      userId,
      address,
    });
